...
const setBusy = b => { $card.setAttribute("aria-busy", b); $card.classList.toggle("loading", b); };
...
async function fetchPokemon(query){
  ...
  // multi-keys cache (id + name)
  CACHE.set(String(p.id), p);
  CACHE.set(p.name.toLowerCase(), p);
...
function renderStats(stats){
  $stats.innerHTML = "";
  const labels = { hp:"PV", attack:"Attaque", defense:"Défense", "special-attack":"Sp. Att", "special-defense":"Sp. Def", speed:"Vitesse" };
...
function renderPokemon(p){
  ...
  // fav button state
  $favBtn.setAttribute("aria-pressed", FAVS.has(String(p.id)));
...
function toggleFav(){
  ...
  $favBtn.setAttribute("aria-pressed", FAVS.has(key));
  refreshFavList();
}
...
$favList.addEventListener("click", (e)=>{
  ...
  if(idRemove){ FAVS.delete(idRemove); localStorage.setItem("pokedex-favs", JSON.stringify([...FAVS])); refreshFavList(); }
});
